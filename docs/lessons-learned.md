...
- **跨平台的 bug，回歸測試只有在會犯錯的那台跑才算數**（#5／#6 空包演練，2026-09-16）：
  `.sha256` 旁檔在 Windows 端 pack 時被寫成 CRLF，Linux 收方 `sha256sum -c` 把 `\r` 當成檔名的一部分而失敗。
  修法是寫檔加 `newline=""`，但補的 `assertNotIn(b"\r", ...)` 在 dgxluna 上**永遠是綠的**（Linux 本來就寫 LF），
  真正驗到東西的是實機端在 Windows 上跑同一個測試。所以：跨 OS 的檔案格式問題，測試要指定「在哪一站跑」才有意義，
  而且**凡是另一站會拿 GNU 工具吃的純文字檔一律寫 LF**。
- **空包演練值得跑一次**：不改任何譯文、只走 pack → 通知 → check → unpack → 回報的完整鏈路，
  兩個方向各一次（延遲 40 s／71 s，都在分鐘內）。這輪抓到的兩個 bug（旁檔行尾、回報草稿誤稱「對方沒給雜湊」）
  都是真流程跑過才現形的，單元測試與單站操作都碰不到。
