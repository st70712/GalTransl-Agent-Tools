...
## 觸發
`/identify-engine` 確認引擎不在 `engines/` 裡，或既有轉接器對這款遊戲無法沿用且差異太大。
...
```bash
cp -r engines/_template engines/<engine>     # adapter.py / profile.json / NOTES.md / vendor/ 骨架
```
...
7. **profile.json**：contexts 優先級／預設政策、控制碼 value/style/leading 正則、never_export、patch 步驟。
8. **adapter.py**：繼承 `StandardCliAdapter`，實作 `detect / prepare / package / breakage_test`；資料樹不在根目錄就覆寫 `data_dir()`，要傳編碼就覆寫 `encoding_args()`。
9. `$PY agt.py gates <game>` 全綠 → smoke build 交使用者實機 → 才大量翻譯。
